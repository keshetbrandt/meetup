...
function checkUser(email){
  var apiUrl = serverURL + "/checkuser?email=" + email; // Replace with the actual URL
  ...
  try {
    ...
    if (response.getResponseCode() === 200) {
      ...
      if (responseData.status === 'user does not exist'){
        return false;
      }
    } else {
...
function postDilemmaSolution(solution, meetingID) {
  // Replace with the URL of your Flask API endpoint for updating preferences
  ...
  //   // Construct the payload for the POST request
    var payload = {
      email: Session.getActiveUser().getEmail(),
      meetingID: meetingID,
      type: solution.slice(0,-1),
      option: solution.charAt(solution.length - 1),
    };
